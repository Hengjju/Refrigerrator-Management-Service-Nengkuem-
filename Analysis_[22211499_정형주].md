...
| Last Update | |
| Status | Analysis |
| Primary Actor | System Scheduler (시스템 스케줄러) |
...
### 4.3 getStorages() 
-> 유저가 가진 냉장고, 김치냉장고 등의 '저장소 목록'을 리스트 형태로 쫙 가져옵니다
...
### 4.5 expiry date
-> 유통기한이 지나서 시스템이 자동으로 폐기 처리(삭제)할 때,또는 수동으로 식재료를 소진 처리(삭제)할 때 호출되는 독립적인 메서드입니다.
...
### 4.7 getExpiringIngredients()
-> 유통기한이 며칠 안 남은 식재료들만 쏙쏙 뽑아서 보여달라고 요청하는 메서드입니다. (알림이나 대시보드에 뿌려줄 때 씁니다)
...
### 4.9 recommend()
-> 현재 내 냉장고에 있는 식재료들과 이 레시피의 재료를 비교해서 "몇 %나 일치하는지 계산해서 유저에게 추천해줍니다.
